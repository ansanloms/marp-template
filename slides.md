...
段落に脚注の参照を置く[^1]。同じスライドに複数の脚注[^2] を並べても、すべて同じスライドの末尾にまとまる。

長いラベル[^long-label] も使える。参照番号と一覧の番号は `<ol start>` で一致させている。

[^1]: 1 ページ目の最初の脚注である。`a.footnote-backref` の矢印で参照位置に戻る。

[^2]: 1 ページ目の 2 つ目の脚注である。

[^long-label]: ラベルが長くても、表示時には番号で採番し直される。

---

# 脚注 (2 ページ目)

別のスライドでは、そのスライドだけの脚注[^slide2-a] を独立に定義できる。

スライドをまたいでも、それぞれの脚注はそのスライド内[^slide2-b] に閉じる。

[^slide2-a]: 2 ページ目の脚注である。1 ページ目の脚注とは混ざらない。

[^slide2-b]: 2 ページ目の 2 つ目の脚注である。スライド単位で分離されることを示す。
